import { IEquipmentItem } from "./data-interfaces";

/*
* The data here is/may be copyrighted and NOT included in the GPLv3 license.
*/
export const mechISEquipmentMisc: IEquipmentItem[] = [
    {
        uuid: "",
        notes: "Active Beagle Probe and Guardian ECM Suite",
        count: 0,
        name: "EW Equipment",
        alternateName: "",
        tag: "ew-equipment",
        sort: "",
        category: "Miscellaneous Equipment",
        damage: 0,
        damageAero: 0,
        damagePerCluster: 0,
        damageClusters: 0,
        accuracyModifier: 0,
        accuracyModifiier: 0,
        cbills: 500000,
        cbillsOneShot: 0,
        introduced: 3030,
        extinct: 3045,
        reintroduced: 0,
        battleValue: 0,
        battleValueOneShot: 0,
        heat: 0,
        heatAero: 0,
        weight: 7.5,
        range: {
            min: 0,
            short: 0,
            medium: 0,
            long: 0,
            extreme: 0
        },
        space: {
            battlemech: 4,
            protomech: 0,
            combatVehicle: 0,
            supportVehicle: 4,
            aerospaceFighter: 0,
            smallCraft: 0,
            dropShip: 0
        },
        ammoPerTon: 0,
        minAmmoTons: 0,
        explosive: false,
        gauss: false,
        weaponType: [],
        techRating: "",
        unique: false,
        book: "TO",
        page: 0,
        alphaStrike: {
            heat: 0,
            rangeShort: 0,
            rangeMedium: 0,
            rangeLong: 0,
            rangeExtreme: 0,
            tc: false,
            notes: []
        },
        battleValuePerItemDamage: 0,
        requiresHandActuator: false,
        weightDivisor: 0,
        damageDivisor: 0,
        criticalsDivisor: 0,
        variableSize: false,
        isMelee: false,
        costPerItemTon: 0,
        location: "",
        rear: false,
        criticals: 0,
        available: false,
        isAmmo: false,
        isEquipment: true
    },
    {
        uuid: "",
        notes: "",
        count: 0,
        name: "C3 Computer (Slave)",
        alternateName: "",
        tag: "c3-computer-slave",
        sort: "",
        category: "Misc Equipment",
        damage: 0,
        damagePerShot: false,
        damageAero: 0,
        heatAero: 0,
        heatPerShot: false,
        damagePerCluster: 0,
        damageClusters: 0,
        accuracyModifier: 0,
        accuracyModifiier: 0,
        cbills: 250000,
        cbillsOneShot: 0,
        introduced: 3050,
        extinct: 0,
        reintroduced: 0,
        battleValue: 0,
        battleValueOneShot: 0,
        heat: 0,
        weight: 1,
        range: {
            min: 0,
            short: 0,
            medium: 0,
            long: 0,
            extreme: 0
        },
        space: {
            battlemech: 1,
            protomech: 0,
            combatVehicle: 1,
            supportVehicle: 1,
            aerospaceFighter: 0,
            smallCraft: 0,
            dropShip: 0
        },
        ammoPerTon: 0,
        minAmmoTons: 0,
        explosive: false,
        gauss: false,
        weaponType: [],
        techRating: "",
        unique: false,
        book: "TM",
        page: 0,
        alphaStrike: {
            heat: 0,
            rangeShort: 0,
            rangeMedium: 0,
            rangeLong: 0,
            rangeExtreme: 0,
            tc: false,
            notes: []
        },
        battleValuePerItemDamage: 0,
        requiresHandActuator: false,
        weightDivisor: 0,
        damageDivisor: 0,
        criticalsDivisor: 0,
        variableSize: false,
        isMelee: false,
        costPerItemTon: 0,
        location: "",
        rear: false,
        criticals: 0,
        available: false,
        isAmmo: false,
        isEquipment: true
    },
    {
        uuid: "",
        notes: "",
        count: 0,
        name: "C3 Computer (Master)",
        alternateName: "",
        tag: "c3-computer-master",
        sort: "",
        category: "Misc Equipment",
        damage: 0,
        damagePerShot: false,
        damageAero: 0,
        heatAero: 0,
        heatPerShot: false,
        damagePerCluster: 0,
        damageClusters: 0,
        accuracyModifier: 0,
        accuracyModifiier: 0,
        cbills: 1500000,
        cbillsOneShot: 0,
        introduced: 3050,
        extinct: 0,
        reintroduced: 0,
        battleValue: 0,
        battleValueOneShot: 0,
        heat: 0,
        weight: 5,
        range: {
            min: 0,
            short: 0,
            medium: 0,
            long: 0,
            extreme: 0
        },
        space: {
            battlemech: 5,
            protomech: 0,
            combatVehicle: 1,
            supportVehicle: 1,
            aerospaceFighter: 0,
            smallCraft: 0,
            dropShip: 0
        },
        ammoPerTon: 0,
        minAmmoTons: 0,
        explosive: false,
        gauss: false,
        weaponType: [],
        techRating: "",
        unique: false,
        book: "TM",
        page: 0,
        alphaStrike: {
            heat: 0,
            rangeShort: 0,
            rangeMedium: 0,
            rangeLong: 0,
            rangeExtreme: 0,
            tc: false,
            notes: []
        },
        battleValuePerItemDamage: 0,
        requiresHandActuator: false,
        weightDivisor: 0,
        damageDivisor: 0,
        criticalsDivisor: 0,
        variableSize: false,
        isMelee: false,
        costPerItemTon: 0,
        location: "",
        rear: false,
        criticals: 0,
        available: false,
        isAmmo: false,
        isEquipment: true
    },
    {
        uuid: "",
        notes: "",
        count: 0,
        name: "ECM Suite",
        alternateName: "Guardian ECM Suite",
        tag: "ecm-suite",
        sort: "",
        category: "Misc Equipment",
        damage: 0,
        damagePerShot: false,
        damageAero: 0,
        heatAero: 0,
        heatPerShot: false,
        damagePerCluster: 0,
        damageClusters: 0,
        accuracyModifier: 0,
        accuracyModifiier: 0,
        cbills: 200000,
        cbillsOneShot: 0,
        introduced: 3045,
        extinct: 0,
        reintroduced: 0,
        battleValue: 61,
        battleValueOneShot: 0,
        heat: 0,
        weight: 1.5,
        range: {
            min: 0,
            short: 0,
            medium: 0,
            long: 0,
            extreme: 0
        },
        space: {
            battlemech: 2,
            protomech: 0,
            combatVehicle: 1,
            supportVehicle: 2,
            aerospaceFighter: 1,
            smallCraft: 1,
            dropShip: 0
        },
        ammoPerTon: 0,
        minAmmoTons: 0,
        explosive: false,
        gauss: false,
        weaponType: [],
        techRating: "",
        unique: false,
        book: "TM",
        page: 0,
        alphaStrike: {
            heat: 0,
            rangeShort: 0,
            rangeMedium: 0,
            rangeLong: 0,
            rangeExtreme: 0,
            tc: false,
            notes: []
        },
        battleValuePerItemDamage: 0,
        requiresHandActuator: false,
        weightDivisor: 0,
        damageDivisor: 0,
        criticalsDivisor: 0,
        variableSize: false,
        isMelee: false,
        costPerItemTon: 0,
        location: "",
        rear: false,
        criticals: 0,
        available: false,
        isAmmo: false,
        isEquipment: true,
        battleValueDefensive: true
    },
    {
        uuid: "",
        notes: "",
        count: 0,
        name: "CASE",
        alternateName: "case",
        tag: "case",
        sort: "case",
        category: "Misc Equipment",
        damage: 0,
        damagePerShot: false,
        damageAero: 0,
        heatAero: 0,
        heatPerShot: false,
        damagePerCluster: 0,
        damageClusters: 0,
        accuracyModifier: 0,
        accuracyModifiier: 0,
        cbills: 50000,
        cbillsOneShot: 0,
        introduced: 2476,
        extinct: 2840,
        reintroduced: 3036,
        battleValue: 0,
        battleValueOneShot: 0,
        heat: 0,
        weight: 0.5,
        range: {
            min: 0,
            short: 0,
            medium: 0,
            long: 0,
            extreme: 0
        },
        space: {
            battlemech: 1,
            protomech: -1,
            combatVehicle: 1,
            supportVehicle: 1,
            aerospaceFighter: 0,
            smallCraft: -1,
            dropShip: -1
        },
        ammoPerTon: 0,
        minAmmoTons: 0,
        explosive: false,
        gauss: false,
        weaponType: [],
        techRating: "",
        unique: false,
        book: "TM",
        page: 210,
        alphaStrike: {
            heat: 0,
            rangeShort: 0,
            rangeMedium: 0,
            rangeLong: 0,
            rangeExtreme: 0,
            tc: false,
            notes: []
        },
        battleValuePerItemDamage: 0,
        requiresHandActuator: false,
        weightDivisor: 0,
        damageDivisor: 0,
        criticalsDivisor: 0,
        variableSize: false,
        isMelee: false,
        costPerItemTon: 0,
        location: "",
        rear: false,
        criticals: 0,
        available: false,
        isAmmo: false,
        isEquipment: true
    },
    {
        isAmmo: false,
        isEquipment: true,
        isModularArmor: true,
        name: "Modular Armor",
        tag: "modular-armor",
        altNames: ["Modular Armor Pack"],
        sort: "armor, modular",
        category: "Miscellaneous Equipment",
        notes: "One pack per location. Protects its mounted location with 10 armor points before standard armor; torso packs protect only the selected facing.",
        cbills: 15000,
        introduced: 3070,
        extinct: 0,
        reintroduced: 0,
        battleValue: 0,
        heat: 0,
        heatAero: 0,
        weight: 1,
        range: { min: 0, short: 0, medium: 0, long: 0 },
        space: { battlemech: 1, protomech: -1, combatVehicle: 1, supportVehicle: -1, aerospaceFighter: -1, smallCraft: -1, dropShip: -1 },
        ammoPerTon: 0,
        minAmmoTons: 0,
        explosive: false,
        weaponType: [],
        techRating: "e",
        book: "TO",
        page: 281,
        alphaStrike: { heat: 0, rangeShort: 0, rangeMedium: 0, rangeLong: 0, rangeExtreme: 0, tc: false, notes: [] },
        additionalArmor: 10,
        currentAdditionalArmor: 10
    }
]